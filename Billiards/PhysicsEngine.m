...
- (void)step:(CGFloat)dt state:(GameState *)state table:(Table *)table {
  ...
  for (Ball *b in [state balls]) {
    ...
    [self handlePocket:b table:table];
  }

  [self handleBallCollisions:[state balls]];
}
...
- (void)handleWallCollision:(Ball *)b table:(Table *)table {
  ...
  if (p.x - radius < NSMinX(r)) {
    p.x = NSMinX(r) + radius;
    v.x = -v.x * _restitution;
  }

  if (p.x + radius > NSMaxX(r)) {
    p.x = NSMaxX(r) - radius;
    v.x = -v.x * _restitution;
  }

  if (p.y - radius < NSMinY(r)) {
    p.y = NSMinY(r) + radius;
    v.y = -v.y * _restitution;
  }

  if (p.y + radius > NSMaxY(r)) {
    p.y = NSMaxY(r) - radius;
    v.y = -v.y * _restitution;
  }
...
- (void)handleBallCollisions:(NSArray *)balls {
...
- (void)handlePocket:(Ball *)b table:(Table *)table {
  for (NSValue *value in [table pockets]) {
    ...
    if (Vec2Len(Vec2Sub([b position], pocket)) < [table pocketRadius]) {
      if ([b isCueBall]) {
        [b setPosition:Vec2Make(NSMidX([table playRect]), NSMidY([table playRect]))];
